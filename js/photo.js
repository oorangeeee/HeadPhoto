document.addEventListener('DOMContentLoaded', () => {
  ...
  function takePhotoNow() {
    ...
    const sourceWidth = Math.min(
      video.videoWidth,
      (video.videoHeight * widthRatio) / heightRatio,
    );
    ...
    if (photoCount < 4) {
      statusText.textContent = `已拍摄${photoCount}张照片，还需${
        4 - photoCount
      }张`;
    } else {
      // 四张照片都拍完了，合并照片
  ...
  function createFilterPanel() {
    ...
    container.insertBefore(
      filterPanel,
      document.querySelector('.storage-container'),
    );
  ...
  function generateStickers(stickerId) {
    ...
    img.onload = () => {
      ...
      const maxSize =
        stickerOption.maxSize ||
        Math.min(mergedCanvas.width, mergedCanvas.height) * 0.15;
      ...
      for (let i = 0; i < count; i++) {
        ...
        while (!isValidPosition && attempts < maxAttempts) {
          ...
          // 随机生成整个画布范围内的位置
          x =
            margin + Math.random() * (mergedCanvas.width - width - margin * 2);
          y =
            margin +
            Math.random() * (mergedCanvas.height - height - margin * 2);

          // 检查位置是否在中心区域
          const inCenterX = Math.abs(x + width / 2 - centerX) < centerWidth / 2;
          const inCenterY =
            Math.abs(y + height / 2 - centerY) < centerHeight / 2;
